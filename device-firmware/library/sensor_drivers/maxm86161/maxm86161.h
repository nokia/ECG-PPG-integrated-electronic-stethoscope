...
/************
adapted from Silicon Lab
*************/
#define MAXM86161_STATUS_OK   0x00
#define MAXM86161_STATUS_FAIL 0x01


/*******************************************************************************
 ***************   Functions for read and write registers     ********************
 ******************************************************************************/
uint32_t maxm86161_i2c_write_to_register( uint8_t address, uint8_t value);
...
uint8_t maxm86161_identify_part(uint8_t*);
/*******************************************************************************
 *******    max86161 Register and Parameter Bit Definitions  *********************
 ******************************************************************************/
#define MAXM86161_FIFO_CFG_2_TIME_STAMP_EN              0x20 
...
#define MAXM86161_FIFO_CFG_2_FIFO_ROLL_OVER_EN          0x02
#define MAXM86161_FIFO_CFG_2_FIFO_ROLL_OVER_STOP        0x00

#define MAXM86161_INT_ENABLE                             0x01
#define MAXM86161_INT_DISABLE                            0x00

#define MAXM86161_PPG_CFG_ALC_ENABLE                     0x01
#define MAXM86161_PPG_CFG_ALC_DISABLE                    0x00

#define MAXM86161_PPG_CFG_OFFSET_ADD                     0x01
#define MAXM86161_PPG_CFG_OFFSET_NO                      0x00

#define MAXM86161_PPG_SYNC_CONTROL_TIME_STAMP_EN        0X80
#define MAXM86161_PPG_SYNC_CONTROL_DAC_CODE_CHG_TAG     0X40
#define MAXM86161_PPG_SYNC_CONTROL_SW_FORCE_SYNC        0X10

//ADC integration time from14.8 to 117.3 us
...
//PPG ADC range setting in unit of nA
enum maxm86161_ppg_cfg_led_range{
  MAXM86161_PPG_CFG_ADC_RANGE_4k  = 0x00,
  MAXM86161_PPG_CFG_ADC_RANGE_8k  = 0x01,
  MAXM86161_PPG_CFG_ADC_RANGE_16k = 0x02,
  MAXM86161_PPG_CFG_ADC_RANGE_32k = 0x03,
};

//led current range setting
...
enum maxm86161_ppg_cfg_led_settling {
  MAXM86161_PPG_LED_SETLNG_4us  = 0x00,
  MAXM86161_PPG_LED_SETLNG_6us  = 0x01,
  MAXM86161_PPG_LED_SETLNG_8us  = 0x02,
  MAXM86161_PPG_LED_SETLNG_12us = 0x03,
};

//DIG_FILT_SEL, don't know what this exactly does? name related technologies:
    //Code Division Multiplexing (CDM)
    //Frequency Division Multiplexing (FDM)
enum maxm86161_ppg_cfg_digifilter_select {
  MAXM86161_PPG_DIGIFILT_CDM   = 0x00,
  MAXM86161_PPG_DIGIFILT_FDM   = 0x01,
};

enum maxm86161_ppg_cfg_burst_rate {
  MAXM86161_PPG_BURST_RATE_8Hz   = 0x0,
  MAXM86161_PPG_BURST_RATE_32Hz  = 0x1,
  MAXM86161_PPG_BURST_RATE_84Hz  = 0x2,
  MAXM86161_PPG_BURST_RATE_256Hz = 0x3,
};

enum maxm86161_ppg_cfg_burst_mode {
  MAXM86161_PPG_BURST_MODE_OFF   = 0x0,
  MAXM86161_PPG_BURST_MODE_OON   = 0x1,
};
...
//0x13 register shifr
#define MAXM86161_PPG_LED_SETLNG                               6
#define MAXM86161_PPG_DIG_FILT_SEL                             5
#define MAXM86161_PPG_BURST_RATE                               1
#define MAXM86161_PPG_BURST_EN                                 0
...
typedef struct {
  uint32_t ppg0;        ///< ppg1 sample data
  uint32_t ppg1;        ///< ppg2 sample data
  uint32_t ppg2;        ///< ppg3 sample data
} maxm86161_ppg_sample_t;

typedef struct {
  uint32_t data[3]; 
}maxm86161_ppg_data_t;

//typedef struct {
//  uint8_t data[3*128];
//} maxm86161_ppg_bytes_t;
...
typedef struct
{
  uint8_t alc;        ///< Enable/disable ALC (ambient light correction)
  ...
  uint8_t smp_avg;   ///< Set the number of exposure per sample
  uint8_t smp_rate;   ///< Set the effective sampling rate of the PPG sensor
  uint8_t led_settling; /// set LED settling time
  uint8_t dig_filter;  // digital filter selector 
} maxm86161_ppg_cfg_t;
...
typedef struct maxm86161_device_config
{
  ...
  maxm86161_led_range_curr_t led_current_range;
...
ret_code_t maxm86161_read_all_bytes_in_fifo(uint8_t *, uint8_t *, uint8_t *);
#endif
